...
for a in range(1,1001):
    ...
    if viol: break
assert viol==0
print(f"F(a+b)<=F(a)+F(b) 1e6 pairs violations=0 PASS")

# 3. Beta envelope
for s,exp in [(1,0),(2,0),(3,0),(4,1),(5,1),(6,2),(12,6)]:
    assert beta_max(s)==exp
print(f"beta_max 1..12 table PASS s=12 ->6 K3,4 attains")

# 4. K2,r + K3,4 vs K2,6
def k2r(r): return {"V":r+2,"E":2*r,"beta":r-1,"s0":r-1}
for r in range(2,9):
    d=k2r(r); assert d["beta"]==r-1
print("K2,r r=2..8 beta=r-1 PASS")
k34_beta=12-7+1; k26_beta=12-(2+6)+1
...
# 5. REAL 5720 spectral — REPLACES LINES 98-122 fake self-comparison
if np is None:
...
def partitions_of_set(n):
    if n==0: return [[]]
    def gen(k,cur):
        if k==n: yield [set(b) for b in cur]; return
        for i in range(len(cur)):
            cur[i].append(k); yield from gen(k+1,cur); cur[i].pop()
        cur.append([k]); yield from gen(k+1,cur); cur.pop()
    return list(gen(0,[]))
